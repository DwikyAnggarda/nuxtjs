<script setup lang="ts">
    ...
    let rules = ref({
        nameRules: [
            (v: any) => !!v || 'Nama Mata Pelajaran wajib diisi',
            // (v: any) => (v && v.length >= 3) || 'Nama Mapel minimal 3 karakter',
        ],
        codeRules: [
            (v: any) => !!v || 'Kode wajib diisi',
            // (v: any) => (v && v.length >= 2) || 'Kode minimal 2 karakter',
        ]
    })
    onMounted(() => {
        sessionStorage.path = '/mapel'
    ...
    const addData = () => {
        formTitle.value = 'Tambah Data Mata Pelajaran'
        formData.value = {}
        method.value = 'POST'
        dialog.value = true
    ...
    const editData = () => {
        formTitle.value = 'Edit Data Mata Pelajaran'  
        method.value = 'PUT'
    ...
    const submitForm = async () => {
        const formMapelRef = formMapel.value
        let status = await formMapelRef.validate()
        ...
        if (status.valid) {
            disabled.value = true
            let urlSubmit = "/mapel"
            if (method.value == "PUT") urlSubmit = "/mapel/" + formData.value.id
...
<template>
    <v-card class="mx-auto">
        ...
        <v-col>
            <v-card-item>
                <v-card-title class="text-h4" style="line-height: 3rem;">Data Induk Mata Pelajaran</v-card-title>
            </v-card-item>
        </v-col>
        <v-col>
            <v-card-actions>
        ...
        <v-dialog v-model="dialog" persistent max-width="450px">
            <v-card>
                <v-form ref="formMapel" v-model="valid" lazy-validation>
                    <v-card-title>
                        <v-btn
                            @click="dialog = false"
                    ...
                    <v-container>
                        <v-row>
                            <v-col cols="12">
                                <v-text-field label="Nama Mata Pelajaran" v-model="formData.name" :rules="rules.nameRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Kode Mata Pelajaran" v-model="formData.code" :rules="rules.codeRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-checkbox
                                    v-model="isActive"
