...
<script setup lang="ts">
    ...
    let rules = ref({
        ...
        fileRules: [
            (value: any) => value[0].size < 2000000 || 'Images size should be less than 2 MB!'
        ]
    })
    onMounted(() => {
        sessionStorage.path = '/paket-rcu'
    ...
    const addData = () => {
        formTitle.value = 'Tambah Data Paket RCU'
        formData.value = {}
        selected.value = []
        item_chips.value = []
    ...
    const editData = () => {
        formTitle.value = 'Edit Data Paket RCU'  
        method.value = 'PUT'
        ...
        if (rows[0].file_code !== null) {
            ...
            console.log('ga ada gambarnya lo')
            backgroundFile.value = [
                {
                    "size": 0,
                    "name": '',
    ...
    const deleteData = () => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            let paket = rows[0]
            authFetch("/paket_rcu/" + paket.uuid, {
                    method: "DELETE",
                    body: '',
            })
            .then((res) => {
    ...
    const handleClick = async (e:any) => {
        if (e.target.matches(".detail-btn")) {
            router.push({
                path : "/paket-rcu/"+e.target.dataset.code
            })
        }
    }

    const selected_tanggal = (val:any) => {
    ...
    const submitForm = async () => {
        ...
        if (status.valid) {
            disabled.value = true
            loading.value = true
            let urlSubmit = "/paket_rcu"
            if (method.value == "PUT") urlSubmit = "/paket_rcu/" + formData.value.uuid
...
<template>
    <v-card class="mx-auto">
        ...
        <v-col>
            <v-card-item>
                <v-card-title class="text-h4" style="line-height: 3rem;">Data Paket RCU</v-card-title>
            </v-card-item>
        </v-col>
        <v-col>
            <v-card-actions>
        ...
        <v-card-text class="py-0">
            <v-row justify="end">
                <v-col cols="6">
                    <Datepicker @update:modelValue="selected_tanggal" v-model="tanggal" placeholder="Periode" format="dd-MM-yyyy" :enable-time-picker="false" locale="id" style="height: 50px;"></Datepicker>
                </v-col>
            </v-row>
