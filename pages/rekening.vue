<script setup lang="ts">
    ...
    let rules = ref({
        nameRules: [
            (v: any) => !!v || 'Atas nama wajib diisi'
        ],
        bankPrefixRules: [
            (v: any) => !!v || 'Prefix bank wajib diisi'
        ],
        bankNameRules: [
            (v: any) => !!v || 'Nama bank wajib diisi'
        ],
        rekeningRules: [
            (v: any) => (v.length >= 9 && v.length <= 16) && /[0-9-]+/.test(v) || 'Minimal 9 angka, Maksimal 16 angka'
        ]
    })
    onMounted(() => {
        sessionStorage.path = '/rekening'
    ...
    const addData = () => {
        formTitle.value = 'Tambah Data Rekening'
        formData.value = {}
        method.value = 'POST'
        dialog.value = true
    ...
    const editData = () => {
        formTitle.value = 'Edit Data Rekening'  
        method.value = 'PUT'
    ...
    const deleteData = () => {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            let rekening = rows[0]
            authFetch("/rekening/" + rekening.id, {
                method: "DELETE",
            })
            .then((res) => {
                return res.json()
    ...
    const submitForm = async () => {
        const formRekeningRef = formRekening.value
        let status = await formRekeningRef.validate()

        const payloadData = {
            rekening: formData.value.rekening,
            atas_nama: formData.value.atas_nama,
            bank_prefix: formData.value.bank_prefix,
            bank_fullname: formData.value.bank_fullname,
        ...
        if (status.valid) {
            disabled.value = true
            let urlSubmit = "/rekening"
            if (method.value == "PUT") urlSubmit = "/rekening/" + formData.value.id
...
<template>
    <v-card class="mx-auto">
        ...
        <v-row>
            <v-col>
        <v-card-item>
            <v-card-title class="text-h4" style="line-height: 3rem;">Data Rekening</v-card-title>
        </v-card-item>
        </v-col>
        <v-col>
            <v-card-actions>
        ...
        <v-dialog v-model="dialog" persistent max-width="450px">
            <v-card>
                <v-form ref="formRekening" v-model="valid" lazy-validation>
                    <v-card-title>
                        <v-btn
                            @click="dialog = false"
                    ...
                    <v-container>
                        <v-row>
                            <v-col cols="12">
                                <v-text-field label="Rekening" v-model="formData.rekening" :rules="rules.rekeningRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Atas nama" v-model="formData.atas_nama" :rules="rules.nameRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Prefix bank" v-model="formData.bank_prefix" :rules="rules.bankPrefixRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Nama bank" v-model="formData.bank_fullname" :rules="rules.bankNameRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-checkbox
                                    v-model="isActive"
