<script setup lang="ts">
    ...
    let rules = ref({
        nameRules: [
            (v: any) => !!v || 'Nama Kelas wajib diisi'
        ],
        codeRules: [
    ...
    const addData = () => {
        formTitle.value = 'Tambah Data Kelas'
        formData.value = {}
        method.value = 'POST'
        dialog.value = true
    ...
    const editData = () => {
        formTitle.value = 'Edit Data Kelas'  
        method.value = 'PUT'
    ...
    const submitForm = async () => {
        const formKelasRef = formKelas.value
        let status = await formKelasRef.validate()
        ...
        if (status.valid) {
            disabled.value = true
            let urlSubmit = "/kelas"
            if (method.value == "PUT") urlSubmit = "/kelas/" + formData.value.id
...
<template>
    <v-card class="mx-auto">
        ...
        <v-row>
            <v-col>
        <v-card-item>
            <v-card-title class="text-h4" style="line-height: 3rem;">Data Kelas</v-card-title>
        </v-card-item>
        </v-col>
        <v-col>
            <v-card-actions>
        ...
        <v-dialog v-model="dialog" persistent max-width="450px">
            <v-card>
                <v-form ref="formKelas" v-model="valid" lazy-validation>
                    <v-card-title>
                        <v-btn
                            @click="dialog = false"
                    ...
                    <v-container>
                        <v-row>
                            <v-col cols="12">
                                <v-text-field label="Nama Kelas" v-model="formData.name" :rules="rules.nameRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-text-field label="Kode Kelas" v-model="formData.code" :rules="rules.codeRules" clearable required></v-text-field>
                            </v-col>
                            <v-col cols="12">
                                <v-checkbox
                                    v-model="isActive"
